...
-- changeset HMA:f905dd02cab559fc551dd1d75a9614c896006652 stripComments:false logicalFilePath:package_bodies\tapi_app_settings.sql
-- sqlcl_snapshot src/database/hma/package_bodies/tapi_app_settings.pkb:null:f905dd02cab559fc551dd1d75a9614c896006652:create

create or replace package body hma.tapi_app_settings is
   ...
   */
   ...
   --By PK
    ...
    from
        app_settings
    where
        setting_name = app_settings_cur.p_setting_name
    for update;
    ...
    begin
        logger.append_param(l_params, 'p_setting_name', p_setting_name);
     ...
     --Get actual NLS_DATE_FORMAT
      ...
      --Alter session for date columns
        ...
        from
            app_settings
        where
            setting_name = hash.p_setting_name;
    ...
    begin
        ...
        from
            app_settings a
        where
            setting_name = rt.p_setting_name;
    ...
    begin
        ...
        from
            app_settings a
        where
            setting_name = rt_for_update.p_setting_name
        for update;
    ...
    is
        ...
        l_app_settings_rec app_settings_rt;
    begin
        logger.append_param(l_params, 'p_setting_name', tt.p_setting_name);
        logger.log('START', l_scope, null, l_params);
        logger.log('Populating record type from DB', l_scope);
        for c1 in (
            select
            ...
            from
                app_settings a
            where
                setting_name = nvl(tt.p_setting_name, setting_name)
        ) loop
    ...
    begin
        ...
        if nvl(p_ignore_nulls, false) then
            update app_settings
            set
                setting_name = nvl(p_app_settings_rec.setting_name, setting_name),
                setting_value = nvl(p_app_settings_rec.setting_value, setting_value)
            where
                setting_name = upd.p_app_settings_rec.setting_name;

        else
            update app_settings
            set
                setting_name = p_app_settings_rec.setting_name,
    ...
    begin
        ...
        if nvl(p_ignore_nulls, false) then
            ...
            set
                ...
                setting_value = nvl(p_app_settings_rec.setting_value, setting_value)
            where
                rowid = p_app_settings_rec.row_id;
            ...
            if web_upd_rowid.p_app_settings_rec.hash != l_app_settings_rec.hash then
                ...
                if nvl(web_upd_rowid.p_ignore_nulls, false) then
                    ...
                    set
                        ...
                        setting_value = nvl(p_app_settings_rec.setting_value, setting_value)
                    where
                        current of app_settings_rowid_cur;
    ...
    procedure del (
        ...
        l_params logger.tab_param;
    begin
        logger.append_param(l_params, 'p_setting_name', del.p_setting_name);
        logger.log('START', l_scope, null, l_params);
        logger.log('Deleting record', l_scope);
        delete from app_settings
        where
            setting_name = del.p_setting_name;
    ...
    procedure web_del (
        ...
        l_app_settings_rec app_settings_rt;
    begin
        logger.append_param(l_params, 'p_setting_name', web_del.p_setting_name);
        logger.log('START', l_scope, null, l_params);
        logger.log('Deleting record', l_scope);
